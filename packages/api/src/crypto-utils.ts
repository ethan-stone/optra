...
export async function sign(
	...
	options: JwtSignOptions | JwtAlgorithm = 'HS256'
): Promise<string> {
...
export async function verify(
	token: string,
	secret: string | JsonWebKey,
	options: JwtVerifyOptions | JwtAlgorithm = { algorithm: 'HS256' }
	...
	if (tokenParts.length !== 3) return { valid: false, reason: InvalidReason.BAD_JWT };
	...
	try {
		...
		if (payload.exp && payload.exp <= Math.floor(Date.now() / 1000)) return { valid: false, reason: InvalidReason.EXPIRED };
 ...
 */
export function decode<Header = {}>(token: string): JwtData<Header> {
	return {
